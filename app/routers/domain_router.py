...
from ..schemas import DomainCreate, DomainUpdate, DomainRead
...
@router.get("/", response_model=List[DomainRead], dependencies=[Depends(fastapi_users.current_user(active=True, superuser=True))])
...
@router.post("/", response_model=DomainRead, dependencies=[Depends(fastapi_users.current_user(active=True, superuser=True))])
...
@router.put("/{id}", response_model=DomainRead, dependencies=[Depends(fastapi_users.current_user(active=True, superuser=True))])
async def update_domain(id: int, domain: DomainUpdate, db: AsyncSession = Depends(get_db), encryption_service: EncryptionService = Depends(get_encryption_service)):
...
@router.delete("/{id}", dependencies=[Depends(fastapi_users.current_user(active=True, superuser=True))])
...
async def validate_domain_connection(
    ...
    try:
        if not domain.server_url or not domain.username or not domain.password:
            ...
            raise HTTPException(status_code=400, detail="server_url, username і password є обов’язковими для перевірки LDAP")
        ...
        logger.error(f"Непередбачена помилка при перевірці домену {domain.name}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Помилка перевірки домену: {str(e)}")

@router.get("/{id}", response_model=DomainRead, dependencies=[Depends(fastapi_users.current_user(active=True, superuser=True))])
...
@router.post("/scan", response_model=dict, dependencies=[Depends(fastapi_users.current_user(active=True, superuser=True))])
async def scan_domains(
    domain_id: int | None = Query(None, description="ID домену для сканування. Якщо не вказано, скануються всі домени"),
    db: AsyncSession = Depends(get_db),
    encryption_service: EncryptionService = Depends(get_encryption_service)
):
