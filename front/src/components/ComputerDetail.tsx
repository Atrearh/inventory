...
const ComputerDetail: React.FC = () => {
  ...
  const { isAuthenticated,timezone } = useAppContext();
      ...
      onOk: async () => {
        ...
        try {
          const response = await executeScript(
            hostname,
            "uninstall_program.ps1",
            { ProgramName: programName },
          );
          if (!response.Success) {
            throw new Error(
              response.Errors?.join(", ") || response.error || "Unknown error",
            );
          }
  ...
  if (compError) {
    return (
      <div className={styles.error}>
        {t("error")}: {handleApiError(compError).message}
      </div>
    );
  }
  ...
  const lastCheckDate = computer.last_updated
    ? new Date(computer.last_updated)
    : null;
  const daysDiff = lastCheckDate
    ? differenceInDays(new Date(), lastCheckDate)
    : Infinity;
  const lastCheckColor =
    daysDiff <= 7 ? "#52c41a" : daysDiff <= 30 ? "#faad14" : "#ff4d4f";
  ...
  const softwareColumns = [
    {
      ...
      dataIndex: "name",
      key: "name",
      sorter: (a: InstalledSoftwareRead, b: InstalledSoftwareRead) =>
    ...
    { title: t("version"), dataIndex: "DisplayVersion", key: "DisplayVersion" },
    {
      ...
      key: "InstallDate",
      render: (val: string) =>
        formatDateInUserTimezone(val, timezone, "dd.MM.yyyy"),
      sorter: (a: InstalledSoftwareRead, b: InstalledSoftwareRead) =>
        new Date(a.install_date || 0).getTime() -
        new Date(b.install_date || 0).getTime(),
    },
    {
      title: t("actions"),
      key: "actions",
  ...
  const historyColumns = [
    ...
    },
    {
      title: t("details"),
      key: "details",
      render: (_: any, rec: ComponentHistory) => JSON.stringify(rec.data),
    },
    {
      title: t("detected_on"),
      ...
      children: (
        <Card>
          <Table
            ...
            rowKey={(rec) =>
              `${rec.component_type}-${rec.detected_on || ""}-${rec.removed_on || ""}`
            }
            loading={histLoading}
            pagination={{ pageSize: 15 }}
            locale={{ emptyText: t("no_data") }}
