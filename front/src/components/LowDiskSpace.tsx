...
const LowDiskSpace: React.FC<LowDiskSpaceProps> = ({ lowDiskSpace, emptyComponent }) => {
  ...
  const diskColumns = [
    {
      ...
      render: (hostname: string, record: DashboardStats['disk_stats']['low_disk_space'][0]) => (
        <Link to={`/computer/${record.id}`}>{hostname || t('unknown', 'Unknown')}</Link>
      ),
    },
    ...
    },
    {
      ...
      key: 'total_space_gb',
      render: (value: number) => (value !== undefined ? value.toFixed(2) : t('not_available', 'N/A')),
    },
    {
  ...
  if (!lowDiskSpace || lowDiskSpace.length === 0) {
    return emptyComponent || <p style={{ color: 'red' }}>{t('no_low_disk_data', 'No low disk space data')}</p>;
  }
  ...
  return (
    <div style={{ marginTop: '16px' }}>
      <Card>
        <Row gutter={[16, 16]}>
          <Col span={24}>
            <h3>{t('low_disk_space_computers', 'Computers with Low Disk Space')}</h3>
            {filteredLowDiskSpace.length > 0 ? (
              ...
              emptyComponent || <p>{t('no_low_disk_computers', 'No computers with low disk space')}</p>
            )}
          </Col>
        </Row>
      </Card>
