...
from ..models import CheckStatus, Domain, Computer
...
class ADService:
    ...
    async def scan_and_update_ad(self, db: AsyncSession, domain: Domain, batch_size: int = 1000) -> None:
        ...
        try:
            ...
            for ad_computer in computers_from_ad:
                ...
                if db_computer:
                    ...
                    if db_computer_by_hostname:
                        ...
                        logger.debug(f"Комп'ютер {hostname} знайдено за іменем, але з іншим GUID. Оновлюємо GUID з {db_computer_by_hostname.object_guid} на {ad_guid}.")
            ...
            logger.debug(f"Порівняння завершено за {(datetime.utcnow() - start_time).total_seconds():.2f} сек: "
            ...
            for i in range(0, len(computers_to_create), batch_size):
                ...
                try:
                    ...
                    logger.error(f"Помилка цілісності при створенні комп'ютерів: {str(e)}", exc_info=True)
                    for c in batch:
            ...
            logger.error(f"Помилка сканування AD для домену {domain.name}: {str(e)}", exc_info=True)
            await db.rollback()
