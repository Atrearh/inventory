...
const ComputerDetail: React.FC = () => {
    ...
    const roleColumns: TableProps<Role>['columns'] = [
        { title: 'Имя', dataIndex: 'name', key: 'name', sorter: true, sortOrder: sort.key === 'name' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('name') }) },
    ];

    const softwareColumns: TableProps<Software>['columns'] = [
        { title: 'Имя', dataIndex: 'name', key: 'name', sorter: true, sortOrder: sort.key === 'name' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('name') }) },
        { title: 'Версия', dataIndex: 'version', key: 'version', render: (value) => value ?? '-', sorter: true, sortOrder: sort.key === 'version' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('version') }) },
        { title: 'Дата установки', dataIndex: 'install_date', key: 'install_date', render: (value) => (value ? new Date(value).toLocaleString('ru-RU') : '-'), sorter: true, sortOrder: sort.key === 'install_date' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('install_date') }) },
    ];

    const diskColumns: TableProps<Disk>['columns'] = [
        { title: 'ID', dataIndex: 'device_id', key: 'device_id', sorter: true, sortOrder: sort.key === 'device_id' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('device_id') }) },
        { title: 'Объем', dataIndex: 'total_space', key: 'total_space', render: (value) => `${value ?? '-'} MB`, sorter: true, sortOrder: sort.key === 'total_space' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('total_space') }) },
        { title: 'Свободно', dataIndex: 'free_space', key: 'free_space', render: (value, record) => `${value ?? '-'} MB (${value && record.total_space ? ((value / record.total_space) * 100).toFixed(2) : '0'}%)`, sorter: true, sortOrder: sort.key === 'free_space' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('free_space') }) },
    ];
    ...
    const historyColumns: TableProps<ChangeLog>['columns'] = [
        ...
        { title: 'Старое', dataIndex: 'old_value', key: 'old_value', render: (value) => value ?? '-', sorter: true, sortOrder: sort.key === 'old_value' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('old_value') }) },
        { title: 'Новое', dataIndex: 'new_value', key: 'new_value', render: (value) => value ?? '-', sorter: true, sortOrder: sort.key === 'new_value' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('new_value') }) },
        { title: 'Дата', dataIndex: 'changed_at', key: 'changed_at', render: (value) => (value ? new Date(value).toLocaleString('ru-RU') : '-'), sorter: true, sortOrder: sort.key === 'changed_at' ? (sort.sort_order === 'asc' ? 'ascend' : 'descend') : null, onHeaderCell: () => ({ onClick: () => handleSort('changed_at') }) },
    ];
