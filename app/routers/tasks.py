...
async def get_scan_tasks(limit: int = 100, offset: int = 0, db: AsyncSession = Depends(get_db)) -> Tuple[List[ScanTask], int]:
    try:
        ...
        raise HTTPException(
            ...
            detail="Помилка сервера при отриманні задач",
        )


@router.delete("/{task_id}", status_code=status.HTTP_200_OK, response_model=dict)
async def delete_scan_task(task_id: UUID, db: AsyncSession = Depends(get_db)) -> dict:
    try:
        repo = TasksRepository(db)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Помилка сервера при видаленні задачі",
        )
...
async def update_scan_task_state(task_id: UUID, state: str, db: AsyncSession = Depends(get_db)) -> ScanTask:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Помилка сервера при оновленні стану задачі",
        )
