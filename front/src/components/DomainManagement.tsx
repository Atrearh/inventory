...
const DomainManagement: React.FC = () => {
  ...
  const { mutate: validateMutate, isPending: isValidatingConnection } =
    useApiMutation({
  ...
  const { mutate: scanDomainsMutate, isPending: isDomainADScanLoading } =
    useApiMutation({
      ...
      onSuccessCallback: (data: any) => {
        if (data.task_id) {
          ...
          message.success(
            t("scan_all_domains_started", {
              task_ids: data.task_ids.join(", "),
            }),
          );
        }
      },
    ...
    () => [
      {
        ...
        dataIndex: "name",
        key: "name",
        sorter: (a: DomainRead, b: DomainRead) => a.name.localeCompare(b.name),
      },
      {
        title: t("username"),
        dataIndex: "username",
        key: "username",
        sorter: (a: DomainRead, b: DomainRead) =>
        ...
        render: (_: any, record: DomainRead) => (
          <Space>
            <Button
              ...
              aria-label={t("edit_domain", { name: record.name })}
            >
              {t("edit")}
            </Button>
            <Button
            ...
            >
              <Button
                danger
                loading={isDeleting}
                aria-label={t("delete_domain", { name: record.name })}
              >
                {t("delete")}
              </Button>
            </Popconfirm>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            ...
            onClick={() => form.submit()}
            aria-label={
              editingItem ? t("save_domain_changes") : t("create_domain")
            }
          >
