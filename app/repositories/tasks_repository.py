...
class TasksRepository:
    ...
    async def update_scan_task_status(self, task_id: str, status: str, scanned_hosts: int, successful_hosts: int, error: Optional[str] = None):
        try:
    ...
    async def get_scan_tasks(self, limit: int = 100, offset: int = 0) -> Tuple[List[models.ScanTask], int]:
        try:
            ...
            logger.debug(f"Отримано {len(tasks)} задач сканування", extra={"limit": limit, "offset": offset})
            return tasks, total
        except SQLAlchemyError as e:
