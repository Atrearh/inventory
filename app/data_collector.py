...
class ScriptCache:
    ...
    def _load_script(self, file_name: str) -> str:
        ...
        if len(script) > 3000:
            logger.warning(
                f"Скрипт {file_name} великий, розмір: {len(script)}",
    ...
    async def preload_scripts(self):
        ...
        for script_name in [f for f in os.listdir(scripts_dir) if f.endswith(".ps1")]:
            try:
                ...
                script_count += 1
                logger.debug(
                    f"Скрипт {script_name} завантажено в кеш",
                    extra={"script_name": script_name},
                )
            except Exception as e:
                logger.error(
                    f"Помилка завантаження {script_name}: {str(e)}",
                    extra={"script_name": script_name},
                )
...
class WinRMDataCollector:
    ...
    async def _execute_script(
        ...
        try:
            ...
            logger.error(
                f"Помилка виконання скрипта: {str(e)}",
        ...
        if len(command) > 3000:
            logger.warning(
                f"Команда для {script_name} на {self.hostname} перевищує 3000 символів",
                extra={"hostname": self.hostname, "script_name": script_name},
            )
        ...
        try:
            async with winrm_service.create_session(self.hostname) as session:
                ...
                if result.status_code != 0:
                    ...
                    logger.error(
                        f"Скрипт {script_name} на {self.hostname} завершився з помилкою, код: {result.status_code}, помилка: {error_message}",
                        extra={"hostname": self.hostname, "script_name": script_name},
                    )
            ...
            logger.error(
                f"Непередбачена помилка при виконанні скрипта {script_name} на {self.hostname}: {str(e)}",
                extra={"hostname": self.hostname, "script_name": script_name},
            )
            raise

    async def collect_pc_info(
        self,
        mode: str = "Full",
        last_updated: Optional[datetime] = None,
        ...
        logger.info(
            f"Початок збору даних для {self.hostname}",
            extra={"hostname": self.hostname},
        )
        ...
        try:
            ...
            if isinstance(hardware_data, dict) and "error" not in hardware_data:
                ...
                logger.debug(
                    "Дані system_info.ps1 зібрано успішно",
                    extra={"hostname": self.hostname},
                )
            else:
                failed_components += 1
            ...
            if isinstance(disk_data, dict) and "error" not in disk_data:
                ...
                successful_components += 1
                logger.debug(
                    "Дані disk_info.ps1 зібрано успішно",
                    extra={"hostname": self.hostname},
                )
            else:
                failed_components += 1
            ...
            if successful_components > 0 and failed_components > 0:
                ...
                logger.info(
                    f"Частково успішний збір даних: {successful_components} успішних, {failed_components} невдалих компонентів",
                    extra={"hostname": self.hostname},
                )
                ...
                logger.warning(
                    "Не вдалося зібрати ключові дані, статус: unreachable",
                    extra={"hostname": self.hostname},
                )
            ...
            logger.error(
                f"Критична помилка при зборі даних: {str(e)}",
                extra={"hostname": self.hostname},
            )
