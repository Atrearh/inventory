...
const HardwareInfo: React.FC<HardwareInfoProps> = ({ computer }) => {
  ...
  const sortByDeletion = <T extends { removed_on?: string | null }>(
    data: T[],
  ): T[] => {
  ...
  const physicalDiskColumns = [
    ...
    {
      title: t("serial_number", "Серийный номер"),
      dataIndex: "serial",
      key: "serial",
    },
    {
      title: t("interface", "Интерфейс"),
      dataIndex: "interface",
      key: "interface",
    },
    {
      ...
      render: (removedOn: string | null) =>
        removedOn ? (
          <Tag color="red">
            {t("deleted", "Видалено")} (
            {new Date(removedOn).toLocaleDateString()})
          </Tag>
        ) : (
  ...
  const videoCardColumns = [
    { title: t("name", "Название"), dataIndex: "Name", key: "Name" }, // Змінено dataIndex на 'Name' відповідно до типу
    {
      ...
      render: (val: number | null) =>
        val ? `${(val / 1024 ** 3).toFixed(2)} ${t("gb", "ГБ")}` : "-",
    },
    {
      title: t("status", "Статус"),
      dataIndex: "removed_on",
    ...
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
      {!computer.is_virtual && (
        <Card title={t("physical_disks", "Физические диски")}>
          <Table
            ...
            rowKey="serial"
            pagination={
              computer.physical_disks && computer.physical_disks.length > 5
                ? { pageSize: 5 }
                : false
            }
            size="small"
      ...
      <Card title={t("logical_disks", "Логические диски")}>
        <Table
          ...
          rowKey="device_id"
          pagination={
            computer.logical_disks && computer.logical_disks.length > 5
              ? { pageSize: 5 }
              : false
          }
          size="small"
      ...
      {!computer.is_virtual && (
        <Card title={t("video_cards", "Видеокарты")}>
          <Table
            ...
            rowKey="Name"
            pagination={
              computer.video_cards && computer.video_cards.length > 5
                ? { pageSize: 5 }
                : false
            }
            size="small"
