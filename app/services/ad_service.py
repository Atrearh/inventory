...
class ADService:
    ...
    def get_ad_computers(self, domain: Domain) -> List[dict]:
        ...
        if not all(
            ...
            raise ValueError(
                "Усі поля (server_url, username, password, ad_base_dn) є обов'язковими для підключення до LDAP"
            )
        ...
        try:
            ...
            logger.error(
                f"Помилка дешифрування пароля для домену {domain.name}: {str(e)}"
            )
        ...
        try:
            conn = Connection(
                server, user=domain.username, password=password, auto_bind=True
            )
            base_dn = domain.ad_base_dn
            logger.debug(
                f"Виконується пошук з base DN: {base_dn}, filter: (objectClass=computer)"
            )
            ...
            for entry in conn.entries:
                ...
                object_guid = (
                    str(entry.objectGUID.value).strip("{}")
                    if entry.objectGUID
                    else None
                )
                if not object_guid:
                ...
                computers.append(
                    {
                        ...
                        "os_name": (
                            entry.operatingSystem.value
                            if entry.operatingSystem
                            else None
                        ),
                        "object_guid": object_guid,
                        "when_created": (
                            entry.whenCreated.value if entry.whenCreated else None
                        ),
                        "when_changed": (
                            entry.whenChanged.value if entry.whenChanged else None
                        ),
                        "enabled": enabled,
                        "ad_notes": (
                            entry.description.value if entry.description else None
                        ),
                        "last_logon": (
                            entry.lastLogon.value if entry.lastLogon else None
                        ),
                        "domain_id": domain.id,
                    }
                )
                logger.debug(
                    f"Знайдено хост: {dns_hostname}, enabled: {enabled}, object_guid: {object_guid}"
                )
            logger.info(
                f"Знайдено {len(computers)} комп'ютерів у AD для домену {domain.name}"
            )
            return computers
        except Exception as e:
            logger.error(
    ...
    async def _compare_computer_data(
        self, db_computer: Computer, ad_data: Dict
    ) -> Tuple[bool, Dict]:
    ...
    async def _get_computer_by_hostname_and_domain(
        self, db: AsyncSession, hostname: str, domain_id: int
    ) -> Optional[Computer]:
        """Отримує комп'ютер за hostname та domain_id."""
        logger.debug(
            f"Перевірка наявності комп'ютера з hostname={hostname} і domain_id={domain_id}"
        )
        try:
            return await self.repo.get_computer_by_hostname_and_domain(
                db, hostname, domain_id
            )
        except Exception as e:
            logger.error(
    ...
    async def _check_duplicate_hostnames_in_ad(
        self, computers: List[Dict], domain: Domain
    ) -> List[Dict]:
        ...
        for hostname, comp_list in hostname_count.items():
            if len(comp_list) > 1:
                logger.warning(
                    f"Знайдено {len(comp_list)} дублікатів hostname {hostname} у домені {domain.name}"
                )
                # Вибираємо найновіший запис за when_changed
                latest_computer = max(
                    comp_list, key=lambda c: c["when_changed"] or datetime.min
                )
                logger.info(
                    f"Вибрано найновіший запис для {hostname}: GUID={latest_computer['object_guid']}"
                )
    ...
    async def scan_and_update_ad(
        self, db: AsyncSession, domain: Domain, batch_size: int = 1000
    ) -> None:
        """Асинхронно сканує AD та оновлює комп'ютери в БД з пакетною обробкою."""
        logger.info(
            f"Початок сканування та оновлення AD для домену {domain.name}, batch_size={batch_size}"
        )
        ...
        try:
            ...
            computers_from_ad = await self._check_duplicate_hostnames_in_ad(
                computers_from_ad, domain
            )
            ad_guids = {c["object_guid"] for c in computers_from_ad}
            ad_computers_dict = {c["object_guid"]: c for c in computers_from_ad}
            logger.debug(
                f"Отримано {len(computers_from_ad)} унікальних комп'ютерів з AD за {(datetime.utcnow() - start_time).total_seconds():.2f} сек"
            )

            # Крок 2: Отримуємо дані з БД
            start_time = datetime.utcnow()
            existing_computers = await self.repo.get_all_computers_by_domain_id(
                db, domain_id=domain.id
            )
            db_computers_dict = {
                c.object_guid: c for c in existing_computers if c.object_guid
            }
            db_hostname_domain_dict = {
                (c.hostname.lower(), c.domain_id): c for c in existing_computers
            }
            logger.debug(
                f"Отримано {len(existing_computers)} комп'ютерів з БД за {(datetime.utcnow() - start_time).total_seconds():.2f} сек"
            )
            ...
            for ad_computer in computers_from_ad:
                ...
                if db_computer:
                    ...
                    has_changes, update_data = await self._compare_computer_data(
                        db_computer, computer_data
                    )
                    if has_changes:
                        logger.debug(
                            f"Комп'ютер {hostname} (GUID: {ad_guid}) має зміни і буде оновлено."
                        )
                        computers_to_update.append((db_computer, update_data))
                else:
                    # Не знайдено за GUID, шукаємо за hostname + domain_id
                    db_computer_by_hostname = db_hostname_domain_dict.get(
                        (hostname, domain_id)
                    )
                    if db_computer_by_hostname:
                        ...
                        has_changes, update_data = await self._compare_computer_data(
                            db_computer_by_hostname, computer_data
                        )
                        computers_to_update.append(
                            (db_computer_by_hostname, update_data)
                        )
                    else:
                        # Не знайдено ні за GUID, ні за іменем. Це новий комп'ютер.
                        logger.debug(
                            f"Комп'ютер {hostname} (GUID: {ad_guid}) буде створено."
                        )
            ...
            for db_computer in existing_computers:
                if db_computer.object_guid and db_computer.object_guid not in ad_guids:
                    logger.debug(
                        f"Комп'ютер {db_computer.hostname} (GUID: {db_computer.object_guid}) видалено з AD"
                    )
            ...
            for i in range(0, len(computers_to_create), batch_size):
                ...
                try:
                    ...
                    for c in batch:
                        existing_computer = (
                            await self._get_computer_by_hostname_and_domain(
                                db, c["hostname"], c["domain_id"]
                            )
                        )
                        if existing_computer:
                            logger.warning(
                                f"Дубльований комп'ютер: {c['hostname']} (domain_id={c['domain_id']}), оновлюємо замість створення"
                            )
                            has_changes, update_data = (
                                await self._compare_computer_data(existing_computer, c)
                            )
                            if (
                                has_changes
                                or existing_computer.object_guid != c["object_guid"]
                            ):
                                ...
                                computers_to_update.append(
                                    (existing_computer, update_data)
                                )
                        else:
                            logger.error(
                                f"Невідома помилка для {c['hostname']} (domain_id={c['domain_id']}): {str(e)}"
                            )
                    continue  # Продовжуємо обробку інших пакетів

            logger.debug(
                f"Створення завершено за {(datetime.utcnow() - start_time).total_seconds():.2f} сек"
            )
            ...
            for db_computer, update_data in computers_to_update:
                logger.info(
                    f"Оновлення комп'ютера: {db_computer.hostname} (ID: {db_computer.id})"
                )
                for key, value in update_data.items():
                    setattr(db_computer, key, value)
                db_computer.last_updated = datetime.utcnow()
            logger.debug(
                f"Оновлення завершено за {(datetime.utcnow() - start_time).total_seconds():.2f} сек"
            )
            ...
            logger.debug(
                f"Видалення завершено за {(datetime.utcnow() - start_time).total_seconds():.2f} сек"
            )
