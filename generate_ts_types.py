        ...
        for module_name in model_modules:
            ...
            try:
                spec = importlib.util.spec_from_file_location(
                    module_name,
                    os.path.join(project_root, module_name.replace(".", os.sep) + ".py"),
                )
                if spec is None:
                ...
                model_classes = [name for name, obj in inspect.getmembers(module, is_pydantic_or_sqlmodel)]

                # Перевіряємо перелічувані типи (Enum)
                enum_classes = [name for name, obj in inspect.getmembers(module, is_enum) if name not in processed_enums]
            ...
            for temp_file in temp_files:
                with open(temp_file, "r", encoding="utf-8") as tf:
                    ...
                    filtered_lines = [
                        line
                        for line in lines
                        if not any(enum_name in line for enum_name in processed_enums) or module_name == "app.models"  # Зберігаємо enum лише з app.models
                    ]
                    final_file.write("\n".join(filtered_lines) + "\n")
                os.unlink(temp_file)
