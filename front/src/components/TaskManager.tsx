...
import { Table, Button, Tag, Progress, Popconfirm, Space, Typography, Tooltip } from 'antd';
import { ReloadOutlined, DeleteOutlined, CheckCircleOutlined, CloseCircleOutlined, SyncOutlined } from '@ant-design/icons';
...
const TaskManager: React.FC = () => {
  ...
  const { mutate: deleteTaskMutation } = useApiMutation< { ok: boolean }, string>({
  ...
  const getStatusTag = (status: ScanStatus) => {
    switch (status) {
      case 'completed':
        return <Tag icon={<CheckCircleOutlined />} color="success">{t('status_completed')}</Tag>;
      case 'running':
        return <Tag icon={<SyncOutlined spin />} color="processing">{t('status_running')}</Tag>;
      case 'failed':
        return <Tag icon={<CloseCircleOutlined />} color="error">{t('status_failed')}</Tag>;
      case 'pending':
  ...
  const columns = [
    {
      ...
      dataIndex: 'id',
      key: 'id',
      render: (id: string) => <Tooltip title={id}><code>{id.substring(0, 8)}...</code></Tooltip>,
    },
    {
      title: t('status'),
      dataIndex: 'status',
    ...
    },
    {
      ...
      dataIndex: 'error',
      key: 'error',
      render: (error: string) => error ? <Tooltip title={error}><span style={{ color: 'red' }}>{t('yes')}</span></Tooltip> : t('no'),
    },
    {
      title: t('actions'),
      key: 'actions',
