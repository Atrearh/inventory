...
import ActionPanel from './ActionPanel'; // ✨ Импорт
import HardwareInfo from './HardwareInfo'; // ✨ Импорт
...
const ComputerDetail: React.FC = () => {
  ...
  let lastCheckColor = daysDiff <= 7 ? '#52c41a' : daysDiff <= 30 ? '#faad14' : '#ff4d4f';

  const softwareColumns = [
    { title: 'Название', dataIndex: 'DisplayName', key: 'DisplayName', sorter: (a: Software, b: Software) => a.DisplayName.localeCompare(b.DisplayName) },
    { title: 'Версия', dataIndex: 'DisplayVersion', key: 'DisplayVersion' },
    { title: 'Дата установки', dataIndex: 'InstallDate', key: 'InstallDate', render: (val: string) => val ? new Date(val).toLocaleDateString('uk-UA') : '-', sorter: (a: Software, b: Software) => new Date(a.InstallDate || 0).getTime() - new Date(b.InstallDate || 0).getTime() },
  ];
  ...
  const historyColumns = [
    ...
    { title: 'Дата обнаружения', dataIndex: 'detected_on', key: 'detected_on', render: (val: string) => val ? new Date(val).toLocaleString('uk-UA') : '-' },
    { title: 'Дата удаления', dataIndex: 'removed_on', key: 'removed_on', render: (val: string) => val ? new Date(val).toLocaleString('uk-UA') : '-' },
  ];
      ...
      children: (
        <Card>
          <Table
            ...
            rowKey={(rec, i) => `${rec.component_type}-${i}`}
            loading={histLoading}
            pagination={{ pageSize: 15 }}
          />
        </Card>
