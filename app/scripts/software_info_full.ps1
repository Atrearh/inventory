...
try {
    ...
    foreach ($regPath in "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*", "HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*") {
        Get-ItemProperty -Path $regPath -ErrorAction SilentlyContinue | Where-Object {
            $_.DisplayName -and (($_.Publisher -ne "Microsoft Corporation") -or ($_.DisplayName -notmatch "Update|Security|Office|Hotfix|KB\d{7}"))
        } | ForEach-Object {
