...
class Computer(SQLModel, table=True):
    ...
    check_status: CheckStatus = Field(
        default=CheckStatus.success, sa_column=Column(Enum(CheckStatus), nullable=False)
    )
...
class ScanTask(SQLModel, table=True):
    ...
    status: ScanStatus = Field(
        default=ScanStatus.pending, sa_column_kwargs={"nullable": False}
    )
...
class LogicalDisk(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True, index=True)
    computer_id: int = Field(foreign_key="computers.id", nullable=False)
    physical_disk_id: Optional[int] = Field(
        default=None, foreign_key="physical_disks.id"
    )
    ...
    physical_disk: Optional["PhysicalDisk"] = Relationship(
        back_populates="logical_disks"
    )
...
class IPAddress(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True, index=True)
    computer_id: int = Field(foreign_key="computers.id", nullable=False)
    address: IPAddressStr = Field(
        max_length=45, nullable=False
    )  # Використовуємо IPAddressStr
    detected_on: datetime = Field(
        default_factory=datetime.utcnow, sa_column_kwargs={"server_default": func.now()}
    )
    removed_on: Optional[datetime] = Field(default=None)
...
class MACAddress(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True, index=True)
    computer_id: int = Field(foreign_key="computers.id", nullable=False)
    address: MACAddressStr = Field(
        max_length=17, nullable=False
    )  # Використовуємо MACAddressStr
    detected_on: datetime = Field(
        default_factory=datetime.utcnow, sa_column_kwargs={"server_default": func.now()}
    )
    removed_on: Optional[datetime] = Field(default=None)
...
class Software(SQLModel, table=True):
    ...
    __table_args__ = (
        Index(
            "idx_software_computer_id", "computer_id", "name", "version", unique=True
        ),
    )
    ...
    def validate_version(cls, value: Optional[str]) -> Optional[str]:
        if value:
            ...
            if not re.match(r"^\d+\.\d+\.\d+(-[a-zA-Z0-9]+)?$", value):
                raise ValueError(
                    "version must follow semantic versioning format (e.g., 1.2.3 or 1.2.3-beta)"
                )
        return value
