...
class Computer(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    hostname = Column(String(255), unique=True, nullable=False)
    ip = Column(String(45), nullable=True)  # Відповідає varchar(45)
    os_name = Column(String(255), nullable=True)
    os_version = Column(String(50), nullable=True)  # Відповідає varchar(50)
    cpu = Column(String(255), nullable=True)
    ram = Column(Integer, nullable=True)
    mac = Column(String(17), nullable=True)  # Відповідає varchar(17)
    motherboard = Column(String(255), nullable=True)
    last_boot = Column(DateTime, nullable=True)  # Відповідає DATETIME
    is_virtual = Column(Boolean, nullable=True)
    check_status = Column(Enum(CheckStatus), default=CheckStatus.unreachable, nullable=True)
    last_updated = Column(DateTime, server_default=func.now(), onupdate=func.now())
    roles = relationship("Role", back_populates="computer", cascade="all, delete-orphan")
    disks = relationship("Disk", back_populates="computer", cascade="all, delete-orphan", lazy="selectin")
    ...
    __table_args__ = (
        Index('idx_computer_ip', 'ip'),
        Index('idx_computer_mac', 'mac'),
    )
...
class Disk(Base):
    ...
    computer = relationship("Computer", back_populates="disks", lazy="selectin")
    __table_args__ = (
...
class ChangeLog(Base):
    ...
    __table_args__ = (
        Index('idx_changelog_computer_id', 'computer_id'),
    )
